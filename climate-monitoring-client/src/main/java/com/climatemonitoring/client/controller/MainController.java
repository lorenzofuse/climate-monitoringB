...
public class MainController {
    ...
    @FXML
    private void handleInsertClimateDataOperatorArea() {
        if (currentUser == null) {
            showAlert(Alert.AlertType.ERROR, "Errore", "Accesso negato", "Effettua il login come operatore");
            return;
        }
        ...
        Dialog<Void> dialog = new Dialog<>();
        dialog.setTitle("Inserisci Dati Climatici - Area Operatore");
        dialog.setHeaderText("Inserisci i parametri climatici per un'area di tuo interesse");
        ...
        // ComboBox per le aree di interesse dell'operatore
        ComboBox<String> areaInteresseComboBox = new ComboBox<>();
        DatePicker dataPicker = new DatePicker();
        Spinner<Integer> ventoSpinner = new Spinner<>(0, 300, 0);
        Spinner<Integer> umiditaSpinner = new Spinner<>(0, 100, 50);
        Spinner<Integer> pressioneSpinner = new Spinner<>(900, 1100, 1013);
        ...
        grid.add(new Label("Area di interesse:"), 0, 0);
        grid.add(areaInteresseComboBox, 1, 0);
        grid.add(new Label("Data:"), 0, 1);
        grid.add(dataPicker, 1, 1);
        grid.add(new Label("Vento (km/h):"), 0, 2);
        grid.add(ventoSpinner, 1, 2);
        ...
        // Popola il ComboBox con le aree di interesse dell'operatore
        try {
            List<String> areeInteresse = service.getAreeInteresseOperatore(currentUser.getId());
            areaInteresseComboBox.getItems().addAll(areeInteresse);
        } catch (RemoteException e) {
            showAlert(Alert.AlertType.ERROR, "Errore di caricamento",
                    "Impossibile caricare le aree", "Si è verificato un errore nel caricamento delle aree: " + e.getMessage());
        }
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == insertButtonType) {
                String selectedAreaName = areaInteresseComboBox.getValue();
                LocalDate selectedDate = dataPicker.getValue();

                if (selectedAreaName == null || selectedDate == null) {
                    showAlert(Alert.AlertType.ERROR, "Dati mancanti",
                            "Campi obbligatori", "Seleziona un'area e una data.");
                    return null;
                }
                ...
                try {
                    ...
                    Integer areaId = service.getAreaInteresseId(selectedAreaName);
                    if (areaId == null) {
                        showAlert(Alert.AlertType.ERROR, "Errore",
                                "Area non trovata", "Non è stato possibile trovare l'ID dell'area selezionata.");
                        return null;
                    }
